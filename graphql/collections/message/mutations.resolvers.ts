...
export const createChannel = async (_, args, context) => {
  ...
  if (!tokenUser.error) {
    ...
    const partner = await User.queryOne('id').eq(args.input.channelMemberId).exec()
    let partnername = partner.firstname == null ? '' : partner.firstname
    partnername = partner.lastname == null ? partnername : partnername + ' ' + partner.lastname
    ...
    const currentUser = await User.queryOne('id').eq(tokenUser.sub).exec()
    let ownname = currentUser.firstname == null ? '' : currentUser.firstname
    ownname = currentUser.lastname == null ? ownname : ownname + ' ' + currentUser.lastname
...
export const createMessage = async (_, args, context) => {
  ...
  if (!tokenUser.error) {
    ...
    const channelAccess = await messageHelper.verifyChannelAccess(tokenUser.sub, args.input.channel)
    ...
    if (channelAccess === true) {
      ...
      try {
        ...
        const channel = ablyRest.channels.get('dm:' + args.input.channel)
      ...
      Channelmember.query('channel').eq(args.input.channel).exec(function (err, channelmembers) {
...
export const getFile = async (_, args, context) => {
  ...
  if (!tokenUser.error) {
    ...
    const channelAccess = await messageHelper.verifyChannelAccess(tokenUser.sub, args.channel)
    ...
    if (channelAccess === true) {
      ...
      try {
        ...
        const ssmparams = {
          Name: '/' + process.env.SERVERLESS_STAGE + '/MESSAGE_FILE_KMS_ENCRYPTION_ACCESSSECRET_KEY',
          WithDecryption: true
        }
        ...
        const s3params = {
          Bucket: process.env.SERVERLESS_STAGE === 'production' ? 'files.designed.org' : 'staging.files.designed.org',
    ...
    if (channelAccess === true) {
      ...
      try {
        ...
        const s3params = {
          ...
          ACL: 'private',
          Tagging: 'channel=' + args.channel + '&userid=' + tokenUser.sub.replace('|', '-')
        }
