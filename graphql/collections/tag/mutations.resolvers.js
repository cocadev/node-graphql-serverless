      ...
      try {
        ...
        await Promise.all(
          args.tagsToMerge.forEach(async tag => {
            Tag.delete(tag.id)
            console.log("Tag deleted now update users")
                  ...
                  updatePromises.push(
                    new Promise(async (resolve, reject) => {
                      try {
                        ...
                        const userDynamoDB = await User.queryOne("id")
                          .eq(userId)
                          .exec()
                        ...
                        if (userDynamoDB) {
                          ...
                          if (userDynamoDB.tags) {
                            const newUserTags = userDynamoDB.tags.filter(
                              userTag =>
                                userTag.id !== tag.id &&
                            ...
                            console.log("userDynamoDB.id", userDynamoDB.id)
